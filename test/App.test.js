...
import ReviewsChart from '../client/src/components/ReviewsChart';
...
describe('Summary Component', () => {
  ...
  it('renders information from props correctly', () => {
    const targetNode = wrapper.find('.whatpplsay');
    expect(targetNode.text()).toMatch('10');
  });

  it('renders neighborhood correctly', () => {
    expect(wrapper.find('.header_neighborhood').text()).toMatch('richmond');
  });
});
...
describe('Recommend Component', () => {
  ...
  it('renders the correct icon', () => {
    expect(wrapper.find(FiThumbsDown)).toHaveLength(1);
  });
  it('renders the correct icon', () => {
    expect(wrapper.find(FiThumbsUp)).toHaveLength(0);
  });
});

describe('ReviewsChart Component', () => {
  const props = {
    reviews: [{
  ...
  const wrapper = shallow(<ReviewsChart {...props} />);
  it('renders the correct percentage', () => {
    expect(wrapper.find('.indivRatingInside')).toHaveLength(5);
    // expect(wrapper.find('.indivRatingInside').get(0).props.style[0].width).toHaveProperty('width', '75%');
    // expect(wrapper.find('.indivRatingInside')).to.have.style('left', '4rem');
  });
});
